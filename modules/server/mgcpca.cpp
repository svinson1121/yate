...
bool MGCPWrapper::rtpMessage(Message& msg)
{
    ...
    bool extend = !fini && (msg.getParam(YSTRING("transport")) ||
	msg.getParam(YSTRING("media")) ||
	msg.getParam(YSTRING("localip")) ||
	msg.getParam(YSTRING("localport")) ||
	msg.getParam(YSTRING("remoteip")) ||
	msg.getParam(YSTRING("remoteport")) ||
	msg.getParam(YSTRING("payload")) ||
	msg.getParam(YSTRING("evpayload")) ||
	msg.getParam(YSTRING("format")) ||
	msg.getParam(YSTRING("direction")) ||
	msg.getParam(YSTRING("ssrc")) ||
	msg.getParam(YSTRING("drillhole")) ||
	msg.getParam(YSTRING("autoaddr")) ||
	msg.getParam(YSTRING("anyssrc")));
    if (fini) {
	if (m_connId.null())
	    return true;
	cmd = "DLCX";
	...
	if (m_connId.null()) {
	    ...
	    if (!copyRename(mm->params,"x-mediatype",msg,"media"))
		mm->params.addParam("x-mediatype",m_audio ? "audio" : "image");
	}
...
MGCPSpan::MGCPSpan(const NamedList& params, const char* name, const MGCPEpInfo& ep)
	...
	static_cast<SignallingCircuitGroup*>(params.getObject(YATOM("SignallingCircuitGroup")))),
      ...
      m_rtpForward(false), m_sdpForward(false), m_rtpForcedFwd(false), m_mediaToken(false),
...
bool MGCPSpan::init(const NamedList& params)
{
    ...
    m_mediaToken = config->getBoolValue(YSTRING("media_token"),false);
...
bool MGCPCircuit::setupConn(const char* mode)
{
    ...
    if (mediaToken() && m_callToken)
	mm->params.addParam("x-calltoken",m_callToken);
    if (m_connId) {
	mm->params.addParam("I",m_connId);
	if (m_delayed) {
	    m_delayed = false;
	...
	m_connId = mm->params.getParam(YSTRING("i"));
    if (m_connId.null()) {
	if (create && (mm->code() >= 200) && (mm->code() <= 299))
	    m_needClear = true;
	return false;
    }
...
bool MGCPCircuit::setParam(const String& param, const String& value)
{
    ...
    else if (param == YSTRING("mgcp_token"))
	m_callToken = value;
    else
	return false;
