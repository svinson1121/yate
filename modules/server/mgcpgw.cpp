...
public:
    ...
    inline const String& bridgeToken() const
	{ return m_bridgeToken; }
    inline bool bridgeWaiting() const
	{ return m_bridgeWait; }
    inline bool established() const
	{ return m_established; }
    inline void bridgeWaiting(bool waiting)
	{ m_bridgeWait = waiting; }
    void setBridge(const String& token);
    void clearBridge();
protected:
...
public:
	...
	{ return m_parser; }
    void activate(bool standby);
    bool autoBridgeBusy(const MGCPChan* chan);
    void tryAutoBridge(MGCPChan* chan);
protected:
...
MGCPChan::MGCPChan(const char* connId)
    : Channel(splugin),
      ...
      m_established(false), m_bridgeWait(false)
{
...
void MGCPChan::setBridge(const String& token)
{
    m_bridgeToken = token;
    m_bridgeWait = true;
}

void MGCPChan::clearBridge()
{
    m_bridgeToken.clear();
    m_bridgeWait = false;
}

void MGCPChan::anchorCall()
{
...
void MGCPChan::destroyed()
{
    ...
    if (m_established && m_callId && m_addr.valid()) {
...
void MGCPChan::callAccept(Message& msg)
{
	...
	params.addParam("M",active ? "sendrecv" : "inactive");
    }
    m_established = true;
...
bool MGCPChan::processEvent(MGCPTransaction* tr, MGCPMessage* mm)
{
	...
	if (m_isRtp) {
		...
		tr->setResponse(400,"RTP update failed");
		return true;
	    }
...
bool MGCPChan::initialEvent(MGCPTransaction* tr, MGCPMessage* mm, const MGCPEndpointId& id)
{
    ...
    rqntParams(mm);
    const String* token = mm->params.getParam(YSTRING("x-calltoken"));
    bool autoBridge = token && *token;
    if (autoBridge) {
	setBridge(*token);
	if (splugin.autoBridgeBusy(this)) {
	    clearBridge();
	    tr->setResponse(540);
	    return true;
	}
...
bool MGCPPlugin::autoBridgeBusy(const MGCPChan* chan)
{
    if (!(chan && chan->bridgeToken()))
	return false;
    Lock lock(this);
	...
	if (!(c && (c != chan)))
	    continue;
	if (c->bridgeToken() != chan->bridgeToken())
	    continue;
	if (!c->established())
	    continue;
	count++;
...
void MGCPPlugin::tryAutoBridge(MGCPChan* chan)
{
    if (!(chan && chan->bridgeWaiting() && chan->established()))
	return;
    RefPointer<MGCPChan> peer;
    lock();
    for (ObjList* l = channels().skipNull(); l; l = l->skipNext()) {
	MGCPChan* c = static_cast<MGCPChan*>(l->get());
	if (!(c && (c != chan)))
	    continue;
	if (!c->bridgeWaiting() || !c->established())
	    continue;
	if (c->bridgeToken() != chan->bridgeToken())
	    continue;
	chan->bridgeWaiting(false);
	c->bridgeWaiting(false);
	peer = c;
	break;
    }
    unlock();
	if (!(peer && chan->connect(peer,"mgcp-autobridge"))) {
	    if (peer) {
		lock();
		chan->bridgeWaiting(true);
		peer->bridgeWaiting(true);
		unlock();
		Debug(this,DebugWarn,"Failed to auto-bridge '%s' to '%s'",
		    chan->id().c_str(),peer->id().c_str());
	}
    }
}
