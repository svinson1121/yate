...
void SS7ISUPCall::buildMediaToken(String& dest) const
{
    dest.clear();
    if (!m_circuit)
	return;
    unsigned int opc = m_label.opc().pack(m_label.type());
    unsigned int dpc = m_label.dpc().pack(m_label.type());
    if (opc > dpc) {
	unsigned int tmp = opc;
	opc = dpc;
	dpc = tmp;
    }
    dest << "isup/" << (unsigned int)m_label.type()
	<< "/" << opc
	<< "/" << dpc
	<< "/" << m_circuit->code();
}
...
bool SS7ISUPCall::connectCircuit(const char* special)
{
    ...
    if (m_circuit && !ok) {
	String token;
	buildMediaToken(token);
	if (token)
	    m_circuit->setParam("mgcp_token",token);
