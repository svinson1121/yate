...
// find a transaction
MGCPTransaction* MGCPEngine::findTrans(unsigned int id, bool outgoing, const SocketAddr* addr)
{
    Lock lock(this);
...
bool MGCPEngine::receive(unsigned char* buffer, SocketAddr& addr)
{
	...
	if (msg->isCommand()) {
		...
		if (trList) {
			...
			MGCPTransaction* tr = findTrans(trList[i],false,&addr);
			if (tr)
		    ...
		    MGCPTransaction* tr = findTrans(msg->transactionId(),false,&addr);
		    if (!tr)
	...
	MGCPTransaction* tr = findTrans(msg->transactionId(),outgoing,outgoing ? 0 : &addr);
	if (tr) {
